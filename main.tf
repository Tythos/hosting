...
module "openwebui" {
  ...
  OPENWEBUI_OLLAMA_BASE_URL         = var.OPENWEBUI_OLLAMA_BASE_URL
  OPENWEBUI_HF_TOKEN                = var.OPENWEBUI_HF_TOKEN
}
...
module "traefik" {
  ...
  TRAEFIK_LOG_PATH         = "${var.MOUNTED_VOLUME}/traefik"
  CROWDSEC_BOUNCER_API_KEY = module.crowdsec.CROWDSEC_BOUNCER_API_KEY
}
